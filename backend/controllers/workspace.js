...
const deleteWorkspace = async (req, res) => {
  try {
    ...
    if (userId.toString() != workspace.owner._id.toString()) {
...
const updateWorkspace = async (req, res) => {
  try {
    ...
    await Workspace.findByIdAndUpdate(workspaceId, { name, color, description });
...
const transferWorkspaceOwner = async (req, res) => {
  try {
    ...
    if (userId.toString() != workspace.owner.toString()) {
      return res.status(403).json({ message: "You don't have permission to transfer owner of this workspace." });
    }
    ...
    workspace.members = workspace.members.map((member) => {
      return {
        ...member.toObject(),
        role: member.user._id.toString() === workspace.owner.toString() ? 'owner' : 'member',
      };
    });
    await workspace.save();
    ...
    // ✅ Send notifications to transfer workspace

    const formattedDate = new Date().toLocaleString("en-IN", {
      dateStyle: "medium",
      timeStyle: "short",
    });
    const message = `Workspace ownership for "${workspace.name}" has been transferred to you by ${req.user.name} on ${formattedDate}`;
    ...
    res.status(200).json({ message: "Workspace Transfer successfully" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server error" });
  }
